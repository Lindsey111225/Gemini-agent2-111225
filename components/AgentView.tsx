...
import { BrainIcon, ComplianceIcon, InteractionIcon, EventIcon, DocumentIcon, CogIcon } from './Icons';
...
export const AgentView: React.FC<AgentViewProps> = ({ onRunAgent, agentResults, isLoading, hasContent }) => {
    ...
    const handleRunConfiguredAgent = (configuredAgent: Agent) => {
        onRunAgent(configuredAgent);
        setAgentToConfigure(null); // Close modal on run
    };
        ...
        <>
                        ...
                        {AGENTS.map(agent => (
                            <div key={agent.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-800">
                                <div className="flex justify-between items-start">
                                    ...
                                    <button onClick={() => setAgentToConfigure(agent)} className="flex items-center gap-2 text-sm bg-slate-600 text-white font-semibold py-1.5 px-3 rounded-md hover:bg-slate-500 transition-colors">
            ...
            {agentToConfigure && (
                <AgentConfigModal
                    ...
                    isOpen={!!agentToConfigure}
                    onClose={() => setAgentToConfigure(null)}
                    onRun={handleRunConfiguredAgent}
                    isLoading={isLoading[`agent-${agentToConfigure.id}`]}
                />
            )}
        </>
    );
